...
export class User {
  ...
  async getUser() {
    try {
      const json_1 = await new Promise((resolve, reject) => {
      ...
      // update user
      this.name = json_1.current_user.full_name;
      this.username = json_1.current_user.user_name;
      ...
      this.id = json_1.current_user.id;
      this.email = json_1.current_user.email.email_address;
      this.clinic = {
        id: json_1.current_user.clinic_id,
        name: json_1.current_user.clinic_name,
      };
      this.isBetaUser = json_1.current_user.beta_features;
  ...
  getClinic() {
    new Promise((resolve, reject) => {
      ...
      if (!this.clinic.id)
        reject(new Error(`Clinic ${this.clinic.id} is invalid. Call method getUser() before getClinic().`));
